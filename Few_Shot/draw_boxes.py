# import tkinter as tk
# from tkinter import filedialog, messagebox, simpledialog
# from PIL import Image, ImageTk
# import os
# import json

# # --- Configuration ---
# IMAGE_DISPLAY_SIZE = (500, 500)
# ANNOTATIONS_FILE = "annotations.json"
# PROGRESS_FILE = "progress.json"
# BBOX_COLOR = "red"
# BBOX_WIDTH = 2

# class ExitDialog(tk.Toplevel):
#     """Custom dialog for exit confirmation."""
#     def __init__(self, parent):
#         super().__init__(parent)
#         self.transient(parent)
#         self.title("Confirm Exit")
#         self.geometry("350x150")
#         self.protocol("WM_DELETE_WINDOW", self._on_cancel) # Handle window close button as cancel

#         self.result = "cancel" # Default choice

#         label = tk.Label(self, text="You have unsaved annotations.\nDo you want to save before exiting?", pady=10)
#         label.pack(pady=10)

#         button_frame = tk.Frame(self)
#         button_frame.pack(pady=10)

#         save_exit_btn = tk.Button(button_frame, text="Save and Exit", command=self._on_save_exit, width=12)
#         save_exit_btn.pack(side=tk.LEFT, padx=5)

#         exit_no_save_btn = tk.Button(button_frame, text="Exit without Saving", command=self._on_exit_no_save, width=15)
#         exit_no_save_btn.pack(side=tk.LEFT, padx=5)

#         cancel_btn = tk.Button(button_frame, text="Cancel", command=self._on_cancel, width=8)
#         cancel_btn.pack(side=tk.LEFT, padx=5)

#         self.grab_set() # Make modal
#         self.wait_window() # Wait until dialog is closed

#     def _on_save_exit(self):
#         self.result = "save_exit"
#         self.destroy()

#     def _on_exit_no_save(self):
#         self.result = "exit_no_save"
#         self.destroy()

#     def _on_cancel(self):
#         self.result = "cancel"
#         self.destroy()

#     def get_choice(self):
#         return self.result

# class AnnotationApp:
#     def __init__(self, root):
#         self.root = root
#         self.root.title("Few-Shot Object Detection Annotator")
#         self.root.geometry("1100x750") # Adjusted for 4 images + controls

#         # --- Data ---
#         self.main_folder_path = ""
#         self.subfolders = []
#         self.current_subfolder_index = -1
#         self.current_subfolder_name = ""
        
#         self.annotations_data = {} # Stores all annotations {subfolder_name: {img_name: [[bboxes]]}}
#         self.progress_data = {}   # Stores {"last_completed_subfolder_name": "sf_name"}

#         # Stores PhotoImage objects to prevent garbage collection
#         self.current_photo_images = [None] * 4 
#         # Stores actual image names loaded into canvases
#         self.current_image_names = [None] * 4
#         # Stores canvas item IDs for bounding boxes: {image_name: [id1, id2, ...]}
#         self.drawn_bbox_ids_on_canvas = {} 
#         # Stores bbox coordinates for the current subfolder's images: {image_name: [[x1,y1,x2,y2], ...]}
#         self.current_subfolder_active_annotations = {} 
#         # Drawing state for each canvas: {canvas_widget: {'start_x':0, 'start_y':0, 'active_rect_id':None, 'image_name':'name.jpg'}}
#         self.canvas_drawing_state = {}
        
#         self.unsaved_changes = False

#         # --- UI Elements ---
#         self.image_canvases = []
#         self.status_bar_label = tk.Label(self.root, text="Status: Initializing...", bd=1, relief=tk.SUNKEN, anchor=tk.W)
        
#         self._setup_ui()
#         self._load_initial_state()
#         self.root.protocol("WM_DELETE_WINDOW", self._on_app_close)

#     def _update_status(self, message):
#         self.status_bar_label.config(text=f"Status: {message}")
#         print(f"Status: {message}") # Also print to console for debugging

#     def _setup_ui(self):
#         # Main Frame
#         main_frame = tk.Frame(self.root)
#         main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

#         # Image Display Frame (2x2 grid)
#         image_grid_frame = tk.Frame(main_frame)
#         image_grid_frame.pack(fill=tk.BOTH, expand=True)

#         for i in range(4):
#             canvas = tk.Canvas(image_grid_frame, width=IMAGE_DISPLAY_SIZE[0], height=IMAGE_DISPLAY_SIZE[1], bg="gray", relief=tk.RIDGE, borderwidth=2)
#             row, col = divmod(i, 2)
#             canvas.grid(row=row, column=col, padx=5, pady=5, sticky="nsew")
#             image_grid_frame.grid_rowconfigure(row, weight=1)
#             image_grid_frame.grid_columnconfigure(col, weight=1)
#             self.image_canvases.append(canvas)
            
#             # Initialize drawing state for this canvas
#             self.canvas_drawing_state[canvas] = {'start_x': 0, 'start_y': 0, 'active_rect_id': None, 'image_name': None}

#             # Bind mouse events for drawing
#             canvas.bind("<ButtonPress-1>", lambda event, c=canvas: self._on_canvas_press(event, c))
#             canvas.bind("<B1-Motion>", lambda event, c=canvas: self._on_canvas_drag(event, c))
#             canvas.bind("<ButtonRelease-1>", lambda event, c=canvas: self._on_canvas_release(event, c))

#         # Controls Frame
#         controls_frame = tk.Frame(main_frame, pady=10)
#         controls_frame.pack(fill=tk.X)

#         self.btn_save = tk.Button(controls_frame, text="Save Annotations", command=self._action_save_annotations, width=20)
#         self.btn_save.pack(side=tk.LEFT, padx=5)

#         self.btn_save_next = tk.Button(controls_frame, text="Save and Next Subfolder", command=self._action_save_and_next, width=25)
#         self.btn_save_next.pack(side=tk.LEFT, padx=5)

#         self.btn_clear = tk.Button(controls_frame, text="Clear Current Subfolder Annotations", command=self._action_clear_current_annotations, width=30)
#         self.btn_clear.pack(side=tk.LEFT, padx=5)
        
#         self.btn_prev = tk.Button(controls_frame, text="Previous Subfolder", command=self._action_previous_subfolder, width=20)
#         self.btn_prev.pack(side=tk.LEFT, padx=5)


#         # Status Bar
#         self.status_bar_label.pack(side=tk.BOTTOM, fill=tk.X)

#     def _load_initial_state(self):
#         self._update_status("Loading initial state...")
#         self.main_folder_path = filedialog.askdirectory(title="Select Main Folder Containing Subfolders")
#         if not self.main_folder_path:
#             self._update_status("No main folder selected. Exiting.")
#             messagebox.showerror("Error", "Main folder not selected. Application will now close.")
#             self.root.destroy()
#             return

#         self._update_status(f"Main folder: {self.main_folder_path}")
#         self._discover_subfolders()

#         if not self.subfolders:
#             self._update_status("No subfolders found in the selected main folder.")
#             messagebox.showinfo("Info", "No subfolders found. Please select a valid main folder.")
#             return

#         self._load_annotations_from_file()
#         self._load_progress_from_file()

#         start_index = 0
#         if "last_completed_subfolder_name" in self.progress_data:
#             last_completed = self.progress_data["last_completed_subfolder_name"]
#             try:
#                 completed_idx = self.subfolders.index(last_completed)
#                 start_index = completed_idx + 1
#                 if start_index >= len(self.subfolders):
#                     self._update_status("All subfolders appear to be completed. Starting from the last one for review.")
#                     start_index = len(self.subfolders) -1 # Load last one for review
#                     if start_index < 0: start_index = 0 # if only one or no subfolders
#             except ValueError:
#                 self._update_status(f"Warning: Last completed subfolder '{last_completed}' not found. Starting from the beginning.")
#                 start_index = 0
        
#         if start_index < len(self.subfolders):
#             self._load_subfolder_by_index(start_index)
#         elif self.subfolders: # If start_index is out of bounds but subfolders exist (e.g. all done)
#              self._update_status("All subfolders processed. Loading last subfolder for review or select another.")
#              self._load_subfolder_by_index(len(self.subfolders) - 1)
#         else:
#             self._update_status("No subfolders to load.")
#             messagebox.showinfo("Info", "No subfolders available to annotate.")


#     def _discover_subfolders(self):
#         if not self.main_folder_path or not os.path.isdir(self.main_folder_path):
#             self.subfolders = []
#             return
#         try:
#             self.subfolders = sorted([
#                 d for d in os.listdir(self.main_folder_path)
#                 if os.path.isdir(os.path.join(self.main_folder_path, d))
#             ])
#             self._update_status(f"Found {len(self.subfolders)} subfolders.")
#         except Exception as e:
#             self.subfolders = []
#             self._update_status(f"Error discovering subfolders: {e}")
#             messagebox.showerror("Error", f"Could not read subfolders: {e}")


#     def _clear_display_and_current_data(self):
#         """Clears canvases and resets current annotation data for a new subfolder."""
#         for i in range(4):
#             self.image_canvases[i].delete("all") # Clear canvas
#             self.image_canvases[i].config(bg="gray") # Reset background
#             self.current_photo_images[i] = None
#             self.current_image_names[i] = None
#             # Reset drawing state for this canvas
#             self.canvas_drawing_state[self.image_canvases[i]] = {'start_x': 0, 'start_y': 0, 'active_rect_id': None, 'image_name': None}


#         self.drawn_bbox_ids_on_canvas.clear() 
#         self.current_subfolder_active_annotations.clear()
#         self.unsaved_changes = False


#     def _load_subfolder_by_index(self, subfolder_idx):
#         if not (0 <= subfolder_idx < len(self.subfolders)):
#             self._update_status(f"Subfolder index {subfolder_idx} is out of bounds.")
#             if not self.subfolders:
#                  messagebox.showinfo("Info", "No subfolders available.")
#             elif subfolder_idx >= len(self.subfolders):
#                  messagebox.showinfo("Info", "You have reached the end of the subfolders.")
#             else: # subfolder_idx < 0
#                  messagebox.showinfo("Info", "You are at the beginning of the subfolders.")
#             return

#         self._clear_display_and_current_data()
#         self.current_subfolder_index = subfolder_idx
#         self.current_subfolder_name = self.subfolders[subfolder_idx]
#         full_subfolder_path = os.path.join(self.main_folder_path, self.current_subfolder_name)

#         self._update_status(f"Loading subfolder: {self.current_subfolder_name} ({self.current_subfolder_index + 1}/{len(self.subfolders)})")

#         try:
#             image_files = sorted([
#                 f for f in os.listdir(full_subfolder_path)
#                 if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif'))
#             ])
#         except Exception as e:
#             self._update_status(f"Error reading images from {self.current_subfolder_name}: {e}")
#             messagebox.showerror("Error", f"Could not read images from {self.current_subfolder_name}: {e}")
#             return

#         if len(image_files) == 0:
#             self._update_status(f"No images found in {self.current_subfolder_name}.")
#             messagebox.showwarning("Warning", f"No images found in subfolder: {self.current_subfolder_name}")
#             # Potentially skip this subfolder or allow user to navigate
#             return
        
#         if len(image_files) != 4:
#              self._update_status(f"Warning: Subfolder {self.current_subfolder_name} has {len(image_files)} images, expected 4. Displaying up to 4.")
#              # messagebox.showwarning("Warning", f"Subfolder {self.current_subfolder_name} has {len(image_files)} images, expected 4. Displaying first 4 if available.")


#         # Load annotations for this subfolder if they exist in self.annotations_data
#         subfolder_annotations_from_file = self.annotations_data.get(self.current_subfolder_name, {})

#         for i in range(min(len(image_files), 4)): # Display up to 4 images
#             img_name = image_files[i]
#             img_path = os.path.join(full_subfolder_path, img_name)
#             canvas = self.image_canvases[i]
            
#             try:
#                 img = Image.open(img_path)
#                 img = img.resize(IMAGE_DISPLAY_SIZE, Image.LANCZOS)
#                 photo_img = ImageTk.PhotoImage(img)

#                 self.current_photo_images[i] = photo_img # Keep reference
#                 self.current_image_names[i] = img_name
#                 canvas.create_image(0, 0, anchor=tk.NW, image=photo_img)
#                 canvas.config(bg="white") # Change background once image loaded

#                 # Associate image name with canvas for drawing state
#                 self.canvas_drawing_state[canvas]['image_name'] = img_name
                
#                 # Initialize annotation structures for this image
#                 self.current_subfolder_active_annotations[img_name] = subfolder_annotations_from_file.get(img_name, []).copy() # Deep copy list of lists
#                 self.drawn_bbox_ids_on_canvas[img_name] = []

#                 # Draw existing bboxes
#                 for bbox in self.current_subfolder_active_annotations[img_name]:
#                     rect_id = canvas.create_rectangle(bbox, outline=BBOX_COLOR, width=BBOX_WIDTH)
#                     self.drawn_bbox_ids_on_canvas[img_name].append(rect_id)

#             except Exception as e:
#                 self._update_status(f"Error loading image {img_name}: {e}")
#                 canvas.create_text(IMAGE_DISPLAY_SIZE[0]//2, IMAGE_DISPLAY_SIZE[1]//2, text=f"Error loading\n{img_name}", fill="red", justify=tk.CENTER)
#                 self.current_image_names[i] = None # Mark as not loaded

#         # For any remaining canvases if fewer than 4 images
#         for i in range(len(image_files), 4):
#             self.image_canvases[i].delete("all")
#             self.image_canvases[i].create_text(IMAGE_DISPLAY_SIZE[0]//2, IMAGE_DISPLAY_SIZE[1]//2, text="No Image", fill="black")
#             self.current_image_names[i] = None
#             self.canvas_drawing_state[self.image_canvases[i]]['image_name'] = None


#         self.unsaved_changes = False # Reset as we just loaded
#         self._update_button_states()


#     def _update_button_states(self):
#         can_go_next = self.current_subfolder_index < len(self.subfolders) - 1
#         can_go_prev = self.current_subfolder_index > 0

#         self.btn_save_next.config(state=tk.NORMAL if can_go_next and self.subfolders else tk.DISABLED)
#         self.btn_prev.config(state=tk.NORMAL if can_go_prev and self.subfolders else tk.DISABLED)
#         self.btn_save.config(state=tk.NORMAL if self.subfolders else tk.DISABLED)
#         self.btn_clear.config(state=tk.NORMAL if self.subfolders else tk.DISABLED)


#     # --- Canvas Drawing Event Handlers ---
#     def _on_canvas_press(self, event, canvas):
#         state = self.canvas_drawing_state[canvas]
#         if state['image_name'] is None: # No image on this canvas
#             return
#         state['start_x'] = canvas.canvasx(event.x) # Translate to canvas coordinates
#         state['start_y'] = canvas.canvasy(event.y)
#         # Create a temporary rectangle for dragging
#         state['active_rect_id'] = canvas.create_rectangle(state['start_x'], state['start_y'], state['start_x'], state['start_y'], outline=BBOX_COLOR, width=BBOX_WIDTH, dash=(4, 2))

#     def _on_canvas_drag(self, event, canvas):
#         state = self.canvas_drawing_state[canvas]
#         if state['image_name'] is None or state['active_rect_id'] is None:
#             return
#         cur_x = canvas.canvasx(event.x)
#         cur_y = canvas.canvasy(event.y)
#         canvas.coords(state['active_rect_id'], state['start_x'], state['start_y'], cur_x, cur_y)

#     def _on_canvas_release(self, event, canvas):
#         state = self.canvas_drawing_state[canvas]
#         if state['image_name'] is None or state['active_rect_id'] is None:
#             return

#         end_x = canvas.canvasx(event.x)
#         end_y = canvas.canvasy(event.y)
        
#         # Remove the temporary dashed rectangle
#         canvas.delete(state['active_rect_id'])
#         state['active_rect_id'] = None

#         # Ensure coordinates are top-left and bottom-right
#         x1 = min(state['start_x'], end_x)
#         y1 = min(state['start_y'], end_y)
#         x2 = max(state['start_x'], end_x)
#         y2 = max(state['start_y'], end_y)

#         # Ignore tiny boxes (e.g. accidental clicks)
#         if abs(x1 - x2) < 5 or abs(y1 - y2) < 5:
#             self._update_status("Box too small, not added.")
#             return

#         bbox_coords = [int(x1), int(y1), int(x2), int(y2)]
        
#         # Draw the final solid rectangle
#         final_rect_id = canvas.create_rectangle(bbox_coords, outline=BBOX_COLOR, width=BBOX_WIDTH)

#         # Store annotation
#         current_image_name = state['image_name']
#         if current_image_name not in self.current_subfolder_active_annotations:
#             self.current_subfolder_active_annotations[current_image_name] = []
#         self.current_subfolder_active_annotations[current_image_name].append(bbox_coords)
        
#         if current_image_name not in self.drawn_bbox_ids_on_canvas:
#             self.drawn_bbox_ids_on_canvas[current_image_name] = []
#         self.drawn_bbox_ids_on_canvas[current_image_name].append(final_rect_id)
        
#         self.unsaved_changes = True
#         self._update_status(f"Added bbox to {current_image_name}")


#     # --- Button Actions ---
#     def _action_save_annotations(self, called_from_next=False):
#         if not self.current_subfolder_name:
#             self._update_status("No subfolder loaded to save.")
#             return False # Indicate save failed or not applicable

#         # Update main annotations data structure
#         self.annotations_data[self.current_subfolder_name] = self.current_subfolder_active_annotations.copy() # Store a copy
        
#         # Save annotations to JSON file
#         self._write_json_file(ANNOTATIONS_FILE, self.annotations_data)
        
#         # Update progress
#         self.progress_data["last_completed_subfolder_name"] = self.current_subfolder_name
#         self._write_json_file(PROGRESS_FILE, self.progress_data)
        
#         self.unsaved_changes = False
#         if not called_from_next: # Avoid double status update if called by _action_save_and_next
#             self._update_status(f"Annotations for {self.current_subfolder_name} saved.")
#         return True # Indicate save was successful

#     def _action_save_and_next(self):
#         if self._action_save_annotations(called_from_next=True): # Save current first
#             self._update_status(f"Annotations for {self.current_subfolder_name} saved. Moving to next.")
#             if self.current_subfolder_index < len(self.subfolders) - 1:
#                 self._load_subfolder_by_index(self.current_subfolder_index + 1)
#             else:
#                 self._update_status("All subfolders annotated!")
#                 messagebox.showinfo("Info", "You have reached the end of the subfolders.")
#         else:
#             # Save might have failed or not been applicable (e.g., no subfolder loaded)
#             self._update_status("Could not save, cannot move to next.")


#     def _action_clear_current_annotations(self):
#         if not self.current_subfolder_name:
#             self._update_status("No subfolder loaded to clear.")
#             return

#         if not self.unsaved_changes and not any(self.current_subfolder_active_annotations.values()):
#             self._update_status(f"No annotations to clear for {self.current_subfolder_name}.")
#             return

#         if messagebox.askyesno("Confirm Clear", f"Are you sure you want to clear all annotations for subfolder '{self.current_subfolder_name}'? This action cannot be undone until next save."):
#             for i, canvas in enumerate(self.image_canvases):
#                 img_name = self.current_image_names[i]
#                 if img_name and img_name in self.drawn_bbox_ids_on_canvas:
#                     for rect_id in self.drawn_bbox_ids_on_canvas[img_name]:
#                         canvas.delete(rect_id)
#                     self.drawn_bbox_ids_on_canvas[img_name] = []
#                 if img_name: # Also clear the data structure
#                     self.current_subfolder_active_annotations[img_name] = []
            
#             self.unsaved_changes = True # Mark as changed because data is now different from saved
#             self._update_status(f"Cleared annotations for {self.current_subfolder_name}. Save to persist this.")

#     def _action_previous_subfolder(self):
#         if self.unsaved_changes:
#             if not messagebox.askyesno("Unsaved Changes", "You have unsaved changes. Do you want to proceed to the previous subfolder without saving?"):
#                 return
        
#         if self.current_subfolder_index > 0:
#             self._load_subfolder_by_index(self.current_subfolder_index - 1)
#         else:
#             self._update_status("Already at the first subfolder.")
#             messagebox.showinfo("Info", "You are at the first subfolder.")


#     # --- File I/O ---
#     def _load_annotations_from_file(self):
#         try:
#             if os.path.exists(ANNOTATIONS_FILE):
#                 with open(ANNOTATIONS_FILE, 'r') as f:
#                     self.annotations_data = json.load(f)
#                 self._update_status(f"Loaded annotations from {ANNOTATIONS_FILE}")
#             else:
#                 self.annotations_data = {}
#                 self._update_status(f"{ANNOTATIONS_FILE} not found. Starting fresh.")
#         except Exception as e:
#             self.annotations_data = {}
#             self._update_status(f"Error loading {ANNOTATIONS_FILE}: {e}. Starting fresh.")
#             messagebox.showwarning("Warning", f"Could not load annotations file: {e}")

#     def _load_progress_from_file(self):
#         try:
#             if os.path.exists(PROGRESS_FILE):
#                 with open(PROGRESS_FILE, 'r') as f:
#                     self.progress_data = json.load(f)
#                 self._update_status(f"Loaded progress from {PROGRESS_FILE}")
#             else:
#                 self.progress_data = {}
#                 self._update_status(f"{PROGRESS_FILE} not found. Starting fresh.")
#         except Exception as e:
#             self.progress_data = {}
#             self._update_status(f"Error loading {PROGRESS_FILE}: {e}. Starting fresh.")
#             messagebox.showwarning("Warning", f"Could not load progress file: {e}")

#     def _write_json_file(self, filepath, data):
#         try:
#             with open(filepath, 'w') as f:
#                 json.dump(data, f, indent=2)
#             # self._update_status(f"Data saved to {filepath}") # This can be too verbose
#         except Exception as e:
#             self._update_status(f"Error saving to {filepath}: {e}")
#             messagebox.showerror("Error", f"Could not save to {filepath}: {e}")


#     # --- Exit Handling ---
#     def _on_app_close(self):
#         if self.unsaved_changes:
#             dialog = ExitDialog(self.root)
#             choice = dialog.get_choice()

#             if choice == "save_exit":
#                 self._action_save_annotations()
#                 self.root.destroy()
#             elif choice == "exit_no_save":
#                 self.root.destroy()
#             elif choice == "cancel":
#                 return # Do nothing, keep app open
#         else:
#             self.root.destroy()


# if __name__ == "__main__":
#     root = tk.Tk()
#     app = AnnotationApp(root)
#     root.mainloop()


















# import tkinter as tk
# from tkinter import filedialog, messagebox, simpledialog
# from PIL import Image, ImageTk
# import os
# import json

# # --- Configuration ---
# IMAGE_DISPLAY_SIZE = (500, 500)
# ANNOTATIONS_FILE = "annotations.json"
# PROGRESS_FILE = "progress.json"
# BBOX_COLOR = "red"
# BBOX_WIDTH = 2

# class ExitDialog(tk.Toplevel):
#     """Custom dialog for exit confirmation."""
#     def __init__(self, parent):
#         super().__init__(parent)
#         self.transient(parent)
#         self.title("Confirm Exit")
#         # Increased dialog width slightly for button text
#         self.geometry("380x150") 
#         self.protocol("WM_DELETE_WINDOW", self._on_cancel)

#         self.result = "cancel" 

#         label = tk.Label(self, text="You have unsaved annotations.\nDo you want to save before exiting?", pady=10)
#         label.pack(pady=10)

#         button_frame = tk.Frame(self)
#         button_frame.pack(pady=10)

#         save_exit_btn = tk.Button(button_frame, text="Save and Exit", command=self._on_save_exit, width=15) # Adjusted width
#         save_exit_btn.pack(side=tk.LEFT, padx=5)

#         exit_no_save_btn = tk.Button(button_frame, text="Exit without Saving", command=self._on_exit_no_save, width=18) # Adjusted width
#         exit_no_save_btn.pack(side=tk.LEFT, padx=5)

#         cancel_btn = tk.Button(button_frame, text="Cancel", command=self._on_cancel, width=10) # Adjusted width
#         cancel_btn.pack(side=tk.LEFT, padx=5)

#         self.grab_set() 
#         self.wait_window()

#     def _on_save_exit(self):
#         self.result = "save_exit"
#         self.destroy()

#     def _on_exit_no_save(self):
#         self.result = "exit_no_save"
#         self.destroy()

#     def _on_cancel(self):
#         self.result = "cancel"
#         self.destroy()

#     def get_choice(self):
#         return self.result

# class AnnotationApp:
#     def __init__(self, root):
#         self.root = root
#         self.root.title("Few-Shot Object Detection Annotator")
#         # Increased initial height to accommodate 2x500px images + controls
#         # Min height approx: 2*500 (images) + 2*5 (canvas_padding) + 10 (img_grid_pady) + 50 (controls) + 30 (statusbar) + 20 (main_frame_padding) ~ 1100-1150
#         self.root.geometry("1100x1150") 
#         self.root.minsize(800, 700) # Set a reasonable minimum size

#         # --- Data ---
#         self.main_folder_path = ""
#         self.subfolders = []
#         self.current_subfolder_index = -1
#         self.current_subfolder_name = ""
        
#         self.annotations_data = {} 
#         self.progress_data = {}   

#         self.current_photo_images = [None] * 4 
#         self.current_image_names = [None] * 4
#         self.drawn_bbox_ids_on_canvas = {} 
#         self.current_subfolder_active_annotations = {} 
#         self.canvas_drawing_state = {}
        
#         self.unsaved_changes = False

#         # --- UI Elements ---
#         self.image_canvases = []
#         self.status_bar_label = tk.Label(self.root, text="Status: Initializing...", bd=1, relief=tk.SUNKEN, anchor=tk.W)
        
#         self._setup_ui()
#         self._setup_keyboard_shortcuts() # Add keyboard shortcuts
#         self._load_initial_state()
#         self.root.protocol("WM_DELETE_WINDOW", self._on_app_close)

#     def _update_status(self, message):
#         self.status_bar_label.config(text=f"Status: {message}")
#         print(f"Status: {message}") 

#     def _setup_ui(self):
#         # Status Bar - pack this to root window first, at the bottom
#         self.status_bar_label.pack(side=tk.BOTTOM, fill=tk.X)

#         # Main Frame - fills the space above the status bar
#         main_frame = tk.Frame(self.root)
#         main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)


#         # Controls Frame - packed at the BOTTOM of main_frame
#         # No internal pady in Frame constructor, use pack's pady for spacing
#         controls_frame = tk.Frame(main_frame) 
#         controls_frame.pack(side=tk.BOTTOM, fill=tk.X, pady=(10, 0)) # pady=(top_padding, bottom_padding)

#         self.btn_save = tk.Button(controls_frame, text="Save (Ctrl+S)", command=self._action_save_annotations, width=15) # Adjusted width
#         self.btn_save.pack(side=tk.LEFT, padx=5, pady=5)

#         self.btn_save_next = tk.Button(controls_frame, text="Save & Next (Ctrl+Right)", command=self._action_save_and_next, width=22) # Adjusted width
#         self.btn_save_next.pack(side=tk.LEFT, padx=5, pady=5)

#         self.btn_clear = tk.Button(controls_frame, text="Clear Current (Ctrl+D)", command=self._action_clear_current_annotations, width=22) # Adjusted width
#         self.btn_clear.pack(side=tk.LEFT, padx=5, pady=5)
        
#         self.btn_prev = tk.Button(controls_frame, text="Previous (Ctrl+Left)", command=self._action_previous_subfolder, width=20) # Adjusted width
#         self.btn_prev.pack(side=tk.LEFT, padx=5, pady=5)

#         # Image Display Frame (2x2 grid) - packed at the TOP of main_frame, expands to fill remaining space
#         image_grid_frame = tk.Frame(main_frame)
#         image_grid_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True, pady=(0, 5)) # Add some padding below image grid


#         for i in range(4):
#             canvas = tk.Canvas(image_grid_frame, width=IMAGE_DISPLAY_SIZE[0], height=IMAGE_DISPLAY_SIZE[1], bg="gray", relief=tk.RIDGE, borderwidth=1)
#             row, col = divmod(i, 2)
#             canvas.grid(row=row, column=col, padx=5, pady=5, sticky="nsew")
#             image_grid_frame.grid_rowconfigure(row, weight=1)
#             image_grid_frame.grid_columnconfigure(col, weight=1)
#             self.image_canvases.append(canvas)
            
#             self.canvas_drawing_state[canvas] = {'start_x': 0, 'start_y': 0, 'active_rect_id': None, 'image_name': None}

#             canvas.bind("<ButtonPress-1>", lambda event, c=canvas: self._on_canvas_press(event, c))
#             canvas.bind("<B1-Motion>", lambda event, c=canvas: self._on_canvas_drag(event, c))
#             canvas.bind("<ButtonRelease-1>", lambda event, c=canvas: self._on_canvas_release(event, c))


#     def _setup_keyboard_shortcuts(self):
#         self.root.bind("<Control-s>", lambda event: self._action_save_annotations())
#         self.root.bind("<Control-S>", lambda event: self._action_save_annotations()) # Allow Shift
#         self.root.bind("<Control-Right>", lambda event: self._action_save_and_next())
#         self.root.bind("<Control-Left>", lambda event: self._action_previous_subfolder())
#         self.root.bind("<Control-d>", lambda event: self._action_clear_current_annotations())
#         self.root.bind("<Control-D>", lambda event: self._action_clear_current_annotations()) # Allow Shift
#         self._update_status("Keyboard shortcuts active: Ctrl+S (Save), Ctrl+Right (Save & Next), Ctrl+Left (Prev), Ctrl+D (Clear)")


#     def _load_initial_state(self):
#         self._update_status("Loading initial state...")
#         self.main_folder_path = filedialog.askdirectory(title="Select Main Folder Containing Subfolders")
#         if not self.main_folder_path:
#             self._update_status("No main folder selected. Exiting.")
#             messagebox.showerror("Error", "Main folder not selected. Application will now close.")
#             self.root.destroy()
#             return

#         self._update_status(f"Main folder: {self.main_folder_path}")
#         self._discover_subfolders()

#         if not self.subfolders:
#             self._update_status("No subfolders found in the selected main folder.")
#             messagebox.showinfo("Info", "No subfolders found. Please select a valid main folder.")
#             self._update_button_states() # Disable buttons if no subfolders
#             return

#         self._load_annotations_from_file()
#         self._load_progress_from_file()

#         start_index = 0
#         if "last_completed_subfolder_name" in self.progress_data:
#             last_completed = self.progress_data["last_completed_subfolder_name"]
#             try:
#                 completed_idx = self.subfolders.index(last_completed)
#                 start_index = completed_idx + 1
#                 if start_index >= len(self.subfolders):
#                     self._update_status("All subfolders appear to be completed. Starting from the last one for review.")
#                     start_index = len(self.subfolders) -1 
#                     if start_index < 0: start_index = 0 
#             except ValueError:
#                 self._update_status(f"Warning: Last completed subfolder '{last_completed}' not found. Starting from the beginning.")
#                 start_index = 0
        
#         if start_index < len(self.subfolders):
#             self._load_subfolder_by_index(start_index)
#         elif self.subfolders: 
#              self._update_status("All subfolders processed. Loading last subfolder for review or select another.")
#              self._load_subfolder_by_index(len(self.subfolders) - 1)
#         else:
#             self._update_status("No subfolders to load.")
#             messagebox.showinfo("Info", "No subfolders available to annotate.")
        
#         self._update_button_states()


#     def _discover_subfolders(self):
#         if not self.main_folder_path or not os.path.isdir(self.main_folder_path):
#             self.subfolders = []
#             return
#         try:
#             self.subfolders = sorted([
#                 d for d in os.listdir(self.main_folder_path)
#                 if os.path.isdir(os.path.join(self.main_folder_path, d))
#             ])
#             self._update_status(f"Found {len(self.subfolders)} subfolders.")
#         except Exception as e:
#             self.subfolders = []
#             self._update_status(f"Error discovering subfolders: {e}")
#             messagebox.showerror("Error", f"Could not read subfolders: {e}")


#     def _clear_display_and_current_data(self):
#         for i in range(4):
#             self.image_canvases[i].delete("all") 
#             self.image_canvases[i].config(bg="gray") 
#             self.current_photo_images[i] = None
#             self.current_image_names[i] = None
#             self.canvas_drawing_state[self.image_canvases[i]] = {'start_x': 0, 'start_y': 0, 'active_rect_id': None, 'image_name': None}

#         self.drawn_bbox_ids_on_canvas.clear() 
#         self.current_subfolder_active_annotations.clear()
#         self.unsaved_changes = False


#     def _load_subfolder_by_index(self, subfolder_idx):
#         if not self.subfolders: # Should be caught earlier, but as a safeguard
#             self._update_status("No subfolders available to load.")
#             self._update_button_states()
#             return

#         if not (0 <= subfolder_idx < len(self.subfolders)):
#             self._update_status(f"Subfolder index {subfolder_idx} is out of bounds.")
#             if subfolder_idx >= len(self.subfolders):
#                  messagebox.showinfo("Info", "You have reached the end of the subfolders.")
#             else: 
#                  messagebox.showinfo("Info", "You are at the beginning of the subfolders.")
#             self._update_button_states() # Ensure buttons reflect state
#             return

#         self._clear_display_and_current_data()
#         self.current_subfolder_index = subfolder_idx
#         self.current_subfolder_name = self.subfolders[subfolder_idx]
#         full_subfolder_path = os.path.join(self.main_folder_path, self.current_subfolder_name)

#         self.root.title(f"Annotator - {self.current_subfolder_name} ({self.current_subfolder_index + 1}/{len(self.subfolders)})")
#         self._update_status(f"Loading subfolder: {self.current_subfolder_name} ({self.current_subfolder_index + 1}/{len(self.subfolders)})")

#         try:
#             image_files = sorted([
#                 f for f in os.listdir(full_subfolder_path)
#                 if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif'))
#             ])
#         except Exception as e:
#             self._update_status(f"Error reading images from {self.current_subfolder_name}: {e}")
#             messagebox.showerror("Error", f"Could not read images from {self.current_subfolder_name}: {e}")
#             self._update_button_states()
#             return

#         if len(image_files) == 0:
#             self._update_status(f"No images found in {self.current_subfolder_name}.")
#             messagebox.showwarning("Warning", f"No images found in subfolder: {self.current_subfolder_name}")
#             self._update_button_states()
#             return
        
#         if len(image_files) != 4:
#              self._update_status(f"Warning: Subfolder {self.current_subfolder_name} has {len(image_files)} images, expected 4. Displaying up to 4.")
#              # messagebox.showwarning("Warning", f"Subfolder {self.current_subfolder_name} has {len(image_files)} images, expected 4. Displaying first 4 if available.")

#         subfolder_annotations_from_file = self.annotations_data.get(self.current_subfolder_name, {})

#         for i in range(min(len(image_files), 4)): 
#             img_name = image_files[i]
#             img_path = os.path.join(full_subfolder_path, img_name)
#             canvas = self.image_canvases[i]
            
#             try:
#                 img = Image.open(img_path)
#                 img = img.resize(IMAGE_DISPLAY_SIZE, Image.LANCZOS)
#                 photo_img = ImageTk.PhotoImage(img)

#                 self.current_photo_images[i] = photo_img 
#                 self.current_image_names[i] = img_name
#                 canvas.create_image(IMAGE_DISPLAY_SIZE[0]//2, IMAGE_DISPLAY_SIZE[1]//2, anchor=tk.CENTER, image=photo_img) # Centering image
#                 canvas.config(bg="white") 

#                 self.canvas_drawing_state[canvas]['image_name'] = img_name
                
#                 self.current_subfolder_active_annotations[img_name] = subfolder_annotations_from_file.get(img_name, []).copy() 
#                 self.drawn_bbox_ids_on_canvas[img_name] = []

#                 for bbox in self.current_subfolder_active_annotations[img_name]:
#                     rect_id = canvas.create_rectangle(bbox, outline=BBOX_COLOR, width=BBOX_WIDTH)
#                     self.drawn_bbox_ids_on_canvas[img_name].append(rect_id)

#             except Exception as e:
#                 self._update_status(f"Error loading image {img_name}: {e}")
#                 canvas.create_text(IMAGE_DISPLAY_SIZE[0]//2, IMAGE_DISPLAY_SIZE[1]//2, text=f"Error loading\n{img_name}", fill="red", justify=tk.CENTER)
#                 self.current_image_names[i] = None 

#         for i in range(len(image_files), 4):
#             self.image_canvases[i].delete("all")
#             self.image_canvases[i].create_text(IMAGE_DISPLAY_SIZE[0]//2, IMAGE_DISPLAY_SIZE[1]//2, text="No Image", fill="black")
#             self.current_image_names[i] = None
#             self.canvas_drawing_state[self.image_canvases[i]]['image_name'] = None

#         self.unsaved_changes = False 
#         self._update_button_states()


#     def _update_button_states(self):
#         has_subfolders = bool(self.subfolders)
#         can_go_next = has_subfolders and self.current_subfolder_index < len(self.subfolders) - 1
#         can_go_prev = has_subfolders and self.current_subfolder_index > 0

#         self.btn_save.config(state=tk.NORMAL if has_subfolders else tk.DISABLED)
#         self.btn_save_next.config(state=tk.NORMAL if can_go_next else tk.DISABLED)
#         self.btn_clear.config(state=tk.NORMAL if has_subfolders else tk.DISABLED)
#         self.btn_prev.config(state=tk.NORMAL if can_go_prev else tk.DISABLED)

#     def _on_canvas_press(self, event, canvas):
#         state = self.canvas_drawing_state[canvas]
#         if state['image_name'] is None: 
#             return
#         # Adjust for centered image: coordinates are relative to canvas top-left,
#         # but image might not start at (0,0) if canvas is larger than image (not the case here with sticky="nsew")
#         # or if image is drawn with an offset.
#         # Since image is drawn at IMAGE_DISPLAY_SIZE[0]//2, IMAGE_DISPLAY_SIZE[1]//2 with anchor=tk.CENTER,
#         # the image effectively starts at (0,0) of its own 500x500 area on the canvas.
#         # So, event.x and event.y should be fine if canvas size matches IMAGE_DISPLAY_SIZE.
#         # If canvas can be larger due to grid expansion, we need to be careful.
#         # However, canvas width/height is set to IMAGE_DISPLAY_SIZE.
#         # And sticky="nsew" means canvas expands/shrinks. Coordinates should be relative to canvas top-left.
        
#         state['start_x'] = canvas.canvasx(event.x) 
#         state['start_y'] = canvas.canvasy(event.y)
#         state['active_rect_id'] = canvas.create_rectangle(state['start_x'], state['start_y'], state['start_x'], state['start_y'], outline=BBOX_COLOR, width=BBOX_WIDTH, dash=(4, 2))

#     def _on_canvas_drag(self, event, canvas):
#         state = self.canvas_drawing_state[canvas]
#         if state['image_name'] is None or state['active_rect_id'] is None:
#             return
#         cur_x = canvas.canvasx(event.x)
#         cur_y = canvas.canvasy(event.y)
#         canvas.coords(state['active_rect_id'], state['start_x'], state['start_y'], cur_x, cur_y)

#     def _on_canvas_release(self, event, canvas):
#         state = self.canvas_drawing_state[canvas]
#         if state['image_name'] is None or state['active_rect_id'] is None:
#             return

#         end_x = canvas.canvasx(event.x)
#         end_y = canvas.canvasy(event.y)
        
#         canvas.delete(state['active_rect_id'])
#         state['active_rect_id'] = None

#         x1 = min(state['start_x'], end_x)
#         y1 = min(state['start_y'], end_y)
#         x2 = max(state['start_x'], end_x)
#         y2 = max(state['start_y'], end_y)

#         if abs(x1 - x2) < 5 or abs(y1 - y2) < 5:
#             self._update_status("Box too small, not added.")
#             return

#         # Clamp coordinates to be within canvas/image boundaries (0 to IMAGE_DISPLAY_SIZE-1)
#         x1 = max(0, min(x1, IMAGE_DISPLAY_SIZE[0] -1))
#         y1 = max(0, min(y1, IMAGE_DISPLAY_SIZE[1] -1))
#         x2 = max(0, min(x2, IMAGE_DISPLAY_SIZE[0] -1))
#         y2 = max(0, min(y2, IMAGE_DISPLAY_SIZE[1] -1))
        
#         # Check again after clamping if it's still valid
#         if abs(x1 - x2) < 5 or abs(y1 - y2) < 5:
#             self._update_status("Box too small after clamping, not added.")
#             return

#         bbox_coords = [int(x1), int(y1), int(x2), int(y2)]
#         final_rect_id = canvas.create_rectangle(bbox_coords, outline=BBOX_COLOR, width=BBOX_WIDTH)

#         current_image_name = state['image_name']
#         if current_image_name not in self.current_subfolder_active_annotations:
#             self.current_subfolder_active_annotations[current_image_name] = []
#         self.current_subfolder_active_annotations[current_image_name].append(bbox_coords)
        
#         if current_image_name not in self.drawn_bbox_ids_on_canvas:
#             self.drawn_bbox_ids_on_canvas[current_image_name] = []
#         self.drawn_bbox_ids_on_canvas[current_image_name].append(final_rect_id)
        
#         self.unsaved_changes = True
#         self._update_status(f"Added bbox to {current_image_name}: {bbox_coords}")


#     def _action_save_annotations(self, called_from_next=False):
#         if not self.current_subfolder_name:
#             self._update_status("No subfolder loaded to save.")
#             return False 

#         self.annotations_data[self.current_subfolder_name] = self.current_subfolder_active_annotations.copy() 
#         self._write_json_file(ANNOTATIONS_FILE, self.annotations_data)
        
#         self.progress_data["last_completed_subfolder_name"] = self.current_subfolder_name
#         self._write_json_file(PROGRESS_FILE, self.progress_data)
        
#         self.unsaved_changes = False
#         if not called_from_next: 
#             self._update_status(f"Annotations for {self.current_subfolder_name} saved.")
#         return True 

#     def _action_save_and_next(self):
#         if self._action_save_annotations(called_from_next=True): 
#             self._update_status(f"Annotations for {self.current_subfolder_name} saved. Moving to next.")
#             if self.current_subfolder_index < len(self.subfolders) - 1:
#                 self._load_subfolder_by_index(self.current_subfolder_index + 1)
#             else:
#                 self._update_status("All subfolders annotated!")
#                 messagebox.showinfo("Info", "You have reached the end of the subfolders.")
#                 self._update_button_states() # Update buttons (e.g. disable Next)
#         else:
#             self._update_status("Could not save, cannot move to next.")


#     def _action_clear_current_annotations(self):
#         if not self.current_subfolder_name:
#             self._update_status("No subfolder loaded to clear.")
#             return

#         # Check if there's anything to clear visually or in data
#         no_bboxes_drawn = all(not ids for ids in self.drawn_bbox_ids_on_canvas.values())
#         no_bboxes_in_data = all(not bboxes for bboxes in self.current_subfolder_active_annotations.values())

#         if no_bboxes_drawn and no_bboxes_in_data:
#             self._update_status(f"No annotations to clear for {self.current_subfolder_name}.")
#             return

#         if messagebox.askyesno("Confirm Clear", f"Are you sure you want to clear all annotations for subfolder '{self.current_subfolder_name}'? This action cannot be undone until next save."):
#             for i, canvas in enumerate(self.image_canvases):
#                 img_name = self.current_image_names[i]
#                 if img_name and img_name in self.drawn_bbox_ids_on_canvas:
#                     for rect_id in self.drawn_bbox_ids_on_canvas[img_name]:
#                         canvas.delete(rect_id)
#                     self.drawn_bbox_ids_on_canvas[img_name] = []
#                 if img_name and img_name in self.current_subfolder_active_annotations: 
#                     self.current_subfolder_active_annotations[img_name] = []
            
#             self.unsaved_changes = True 
#             self._update_status(f"Cleared annotations for {self.current_subfolder_name}. Save to persist this.")

#     def _action_previous_subfolder(self):
#         if self.unsaved_changes:
#             if not messagebox.askyesno("Unsaved Changes", "You have unsaved changes. Do you want to proceed to the previous subfolder without saving?"):
#                 return
        
#         if self.current_subfolder_index > 0:
#             self._load_subfolder_by_index(self.current_subfolder_index - 1)
#         else:
#             self._update_status("Already at the first subfolder.")
#             messagebox.showinfo("Info", "You are at the first subfolder.")
#         self._update_button_states()


#     def _load_annotations_from_file(self):
#         try:
#             if os.path.exists(ANNOTATIONS_FILE):
#                 with open(ANNOTATIONS_FILE, 'r') as f:
#                     self.annotations_data = json.load(f)
#                 self._update_status(f"Loaded annotations from {ANNOTATIONS_FILE}")
#             else:
#                 self.annotations_data = {}
#                 self._update_status(f"{ANNOTATIONS_FILE} not found. Starting fresh.")
#         except Exception as e:
#             self.annotations_data = {}
#             self._update_status(f"Error loading {ANNOTATIONS_FILE}: {e}. Starting fresh.")
#             messagebox.showwarning("Warning", f"Could not load annotations file: {e}")

#     def _load_progress_from_file(self):
#         try:
#             if os.path.exists(PROGRESS_FILE):
#                 with open(PROGRESS_FILE, 'r') as f:
#                     self.progress_data = json.load(f)
#                 self._update_status(f"Loaded progress from {PROGRESS_FILE}")
#             else:
#                 self.progress_data = {}
#                 self._update_status(f"{PROGRESS_FILE} not found. Starting fresh.")
#         except Exception as e:
#             self.progress_data = {}
#             self._update_status(f"Error loading {PROGRESS_FILE}: {e}. Starting fresh.")
#             messagebox.showwarning("Warning", f"Could not load progress file: {e}")

#     def _write_json_file(self, filepath, data):
#         try:
#             with open(filepath, 'w') as f:
#                 json.dump(data, f, indent=2)
#         except Exception as e:
#             self._update_status(f"Error saving to {filepath}: {e}")
#             messagebox.showerror("Error", f"Could not save to {filepath}: {e}")


#     def _on_app_close(self):
#         if self.unsaved_changes:
#             dialog = ExitDialog(self.root)
#             choice = dialog.get_choice()

#             if choice == "save_exit":
#                 self._action_save_annotations()
#                 self.root.destroy()
#             elif choice == "exit_no_save":
#                 self.root.destroy()
#             elif choice == "cancel":
#                 return 
#         else:
#             self.root.destroy()


# if __name__ == "__main__":
#     root = tk.Tk()
#     app = AnnotationApp(root)
#     root.mainloop()















import tkinter as tk
from tkinter import filedialog, messagebox, simpledialog, ttk # Added ttk for themed scrollbars
from PIL import Image, ImageTk
import os
import json

# --- Configuration ---
IMAGE_DISPLAY_SIZE = (500, 500) # This is the target size for the image content
ANNOTATIONS_FILE = "annotations.json"
PROGRESS_FILE = "progress.json"
BBOX_COLOR = "red"
BBOX_WIDTH = 2

class ExitDialog(tk.Toplevel):
    """Custom dialog for exit confirmation."""
    def __init__(self, parent):
        super().__init__(parent)
        self.transient(parent)
        self.title("Confirm Exit")
        self.geometry("380x150") 
        self.protocol("WM_DELETE_WINDOW", self._on_cancel)

        self.result = "cancel" 

        label = tk.Label(self, text="You have unsaved annotations.\nDo you want to save before exiting?", pady=10)
        label.pack(pady=10)

        button_frame = tk.Frame(self)
        button_frame.pack(pady=10)

        save_exit_btn = tk.Button(button_frame, text="Save and Exit", command=self._on_save_exit, width=15)
        save_exit_btn.pack(side=tk.LEFT, padx=5)

        exit_no_save_btn = tk.Button(button_frame, text="Exit without Saving", command=self._on_exit_no_save, width=18)
        exit_no_save_btn.pack(side=tk.LEFT, padx=5)

        cancel_btn = tk.Button(button_frame, text="Cancel", command=self._on_cancel, width=10)
        cancel_btn.pack(side=tk.LEFT, padx=5)

        self.grab_set() 
        self.wait_window()

    def _on_save_exit(self):
        self.result = "save_exit"
        self.destroy()

    def _on_exit_no_save(self):
        self.result = "exit_no_save"
        self.destroy()

    def _on_cancel(self):
        self.result = "cancel"
        self.destroy()

    def get_choice(self):
        return self.result

class AnnotationApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Few-Shot Object Detection Annotator")
        self.root.geometry("1100x1150") 
        self.root.minsize(800, 700) 

        # --- Data ---
        self.main_folder_path = ""
        self.subfolders = []
        self.current_subfolder_index = -1
        self.current_subfolder_name = ""
        
        self.annotations_data = {} 
        self.progress_data = {}  

        self.current_pil_images = [None] * 4 # To store original PIL images if needed later
        self.current_photo_images = [None] * 4 
        self.current_image_names = [None] * 4
        self.drawn_bbox_ids_on_canvas = {} 
        self.current_subfolder_active_annotations = {} 
        self.canvas_drawing_state = {}
        
        self.unsaved_changes = False

        # --- UI Elements ---
        self.image_canvases = []
        # self.image_frames = [] # Not strictly needed to store if not accessed later
        # self.h_scrollbars = [] # Not strictly needed to store
        # self.v_scrollbars = [] # Not strictly needed to store
        self.status_bar_label = tk.Label(self.root, text="Status: Initializing...", bd=1, relief=tk.SUNKEN, anchor=tk.W)
        
        self._setup_ui()
        self._setup_keyboard_shortcuts() 
        self._load_initial_state()
        self.root.protocol("WM_DELETE_WINDOW", self._on_app_close)

    def _update_status(self, message):
        self.status_bar_label.config(text=f"Status: {message}")
        print(f"Status: {message}") 

    def _setup_ui(self):
        self.status_bar_label.pack(side=tk.BOTTOM, fill=tk.X)

        main_frame = tk.Frame(self.root)
        main_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        controls_frame = tk.Frame(main_frame) 
        controls_frame.pack(side=tk.BOTTOM, fill=tk.X, pady=(10, 0)) 

        self.btn_save = tk.Button(controls_frame, text="Save (Ctrl+S)", command=self._action_save_annotations, width=15) 
        self.btn_save.pack(side=tk.LEFT, padx=5, pady=5)

        self.btn_save_next = tk.Button(controls_frame, text="Save & Next (Ctrl+Right)", command=self._action_save_and_next, width=22) 
        self.btn_save_next.pack(side=tk.LEFT, padx=5, pady=5)

        self.btn_clear = tk.Button(controls_frame, text="Clear Current (Ctrl+D)", command=self._action_clear_current_annotations, width=22) 
        self.btn_clear.pack(side=tk.LEFT, padx=5, pady=5)
        
        self.btn_prev = tk.Button(controls_frame, text="Previous (Ctrl+Left)", command=self._action_previous_subfolder, width=20) 
        self.btn_prev.pack(side=tk.LEFT, padx=5, pady=5)

        image_grid_frame = tk.Frame(main_frame)
        image_grid_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True, pady=(0, 5))

        for i in range(4):
            row, col = divmod(i, 2)
            
            # Frame to contain canvas and scrollbars
            canvas_container_frame = tk.Frame(image_grid_frame)
            canvas_container_frame.grid(row=row, column=col, padx=5, pady=5, sticky="nsew")
            image_grid_frame.grid_rowconfigure(row, weight=1)
            image_grid_frame.grid_columnconfigure(col, weight=1)

            # Canvas
            canvas = tk.Canvas(canvas_container_frame, bg="gray", relief=tk.RIDGE, borderwidth=1)
            
            # Scrollbars (using ttk for better look, tk.Scrollbar also works)
            h_scroll = ttk.Scrollbar(canvas_container_frame, orient=tk.HORIZONTAL, command=canvas.xview)
            v_scroll = ttk.Scrollbar(canvas_container_frame, orient=tk.VERTICAL, command=canvas.yview)
            
            canvas.config(xscrollcommand=h_scroll.set, yscrollcommand=v_scroll.set)
            
            # Layout canvas and scrollbars within their container frame
            canvas.grid(row=0, column=0, sticky="nsew")
            v_scroll.grid(row=0, column=1, sticky="ns")
            h_scroll.grid(row=1, column=0, sticky="ew")
            
            # Make the canvas expand within its container frame
            canvas_container_frame.grid_rowconfigure(0, weight=1)
            canvas_container_frame.grid_columnconfigure(0, weight=1)
            
            self.image_canvases.append(canvas)
            
            self.canvas_drawing_state[canvas] = {'start_x': 0, 'start_y': 0, 'active_rect_id': None, 'image_name': None}
            canvas.bind("<ButtonPress-1>", lambda event, c=canvas: self._on_canvas_press(event, c))
            canvas.bind("<B1-Motion>", lambda event, c=canvas: self._on_canvas_drag(event, c))
            canvas.bind("<ButtonRelease-1>", lambda event, c=canvas: self._on_canvas_release(event, c))
            
            # Set an initial minimal scrollregion; will be updated when image loads
            canvas.config(scrollregion=(0, 0, 1, 1))


    def _setup_keyboard_shortcuts(self):
        self.root.bind("<Control-s>", lambda event: self._action_save_annotations())
        self.root.bind("<Control-S>", lambda event: self._action_save_annotations()) 
        self.root.bind("<Control-Right>", lambda event: self._action_save_and_next())
        self.root.bind("<Control-Left>", lambda event: self._action_previous_subfolder())
        self.root.bind("<Control-d>", lambda event: self._action_clear_current_annotations())
        self.root.bind("<Control-D>", lambda event: self._action_clear_current_annotations()) 
        self._update_status("Keyboard shortcuts active: Ctrl+S (Save), Ctrl+Right (Save & Next), Ctrl+Left (Prev), Ctrl+D (Clear)")


    def _load_initial_state(self):
        self._update_status("Loading initial state...")
        self.main_folder_path = filedialog.askdirectory(title="Select Main Folder Containing Subfolders")
        if not self.main_folder_path:
            self._update_status("No main folder selected. Exiting.")
            messagebox.showerror("Error", "Main folder not selected. Application will now close.")
            self.root.destroy()
            return

        self._update_status(f"Main folder: {self.main_folder_path}")
        self._discover_subfolders()

        if not self.subfolders:
            self._update_status("No subfolders found in the selected main folder.")
            messagebox.showinfo("Info", "No subfolders found. Please select a valid main folder.")
            self._update_button_states() 
            return

        self._load_annotations_from_file()
        self._load_progress_from_file()

        start_index = 0
        if "last_completed_subfolder_name" in self.progress_data:
            last_completed = self.progress_data["last_completed_subfolder_name"]
            try:
                completed_idx = self.subfolders.index(last_completed)
                start_index = completed_idx + 1
                if start_index >= len(self.subfolders):
                    self._update_status("All subfolders appear to be completed. Starting from the last one for review.")
                    start_index = len(self.subfolders) -1 
                    if start_index < 0: start_index = 0 
            except ValueError:
                self._update_status(f"Warning: Last completed subfolder '{last_completed}' not found. Starting from the beginning.")
                start_index = 0
        
        if start_index < len(self.subfolders):
            self._load_subfolder_by_index(start_index)
        elif self.subfolders: 
            self._update_status("All subfolders processed. Loading last subfolder for review or select another.")
            self._load_subfolder_by_index(len(self.subfolders) - 1)
        else:
            self._update_status("No subfolders to load.")
            messagebox.showinfo("Info", "No subfolders available to annotate.")
        
        self._update_button_states()


    def _discover_subfolders(self):
        if not self.main_folder_path or not os.path.isdir(self.main_folder_path):
            self.subfolders = []
            return
        try:
            self.subfolders = sorted([
                d for d in os.listdir(self.main_folder_path)
                if os.path.isdir(os.path.join(self.main_folder_path, d))
            ])
            self._update_status(f"Found {len(self.subfolders)} subfolders.")
        except Exception as e:
            self.subfolders = []
            self._update_status(f"Error discovering subfolders: {e}")
            messagebox.showerror("Error", f"Could not read subfolders: {e}")


    def _clear_display_and_current_data(self):
        for i in range(4):
            canvas = self.image_canvases[i]
            canvas.delete("all") 
            canvas.config(bg="gray", scrollregion=(0, 0, 1, 1)) # Reset scroll region to minimal
            
            self.current_photo_images[i] = None
            self.current_pil_images[i] = None # Clear PIL image reference too
            self.current_image_names[i] = None
            
            if canvas in self.canvas_drawing_state: # Ensure key exists
                self.canvas_drawing_state[canvas] = {'start_x': 0, 'start_y': 0, 'active_rect_id': None, 'image_name': None}

        self.drawn_bbox_ids_on_canvas.clear() 
        self.current_subfolder_active_annotations.clear()
        self.unsaved_changes = False


    def _load_subfolder_by_index(self, subfolder_idx):
        if not self.subfolders: 
            self._update_status("No subfolders available to load.")
            self._update_button_states()
            return

        if not (0 <= subfolder_idx < len(self.subfolders)):
            self._update_status(f"Subfolder index {subfolder_idx} is out of bounds.")
            if subfolder_idx >= len(self.subfolders):
                messagebox.showinfo("Info", "You have reached the end of the subfolders.")
            else: 
                messagebox.showinfo("Info", "You are at the beginning of the subfolders.")
            self._update_button_states() 
            return

        self._clear_display_and_current_data()
        self.current_subfolder_index = subfolder_idx
        self.current_subfolder_name = self.subfolders[subfolder_idx]
        full_subfolder_path = os.path.join(self.main_folder_path, self.current_subfolder_name)

        self.root.title(f"Annotator - {self.current_subfolder_name} ({self.current_subfolder_index + 1}/{len(self.subfolders)})")
        self._update_status(f"Loading subfolder: {self.current_subfolder_name} ({self.current_subfolder_index + 1}/{len(self.subfolders)})")

        try:
            image_files = sorted([
                f for f in os.listdir(full_subfolder_path)
                if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif'))
            ])
        except Exception as e:
            self._update_status(f"Error reading images from {self.current_subfolder_name}: {e}")
            messagebox.showerror("Error", f"Could not read images from {self.current_subfolder_name}: {e}")
            self._update_button_states()
            return

        if len(image_files) == 0:
            self._update_status(f"No images found in {self.current_subfolder_name}.")
            #messagebox.showwarning("Warning", f"No images found in subfolder: {self.current_subfolder_name}") # This can be noisy
        
        if len(image_files) != 4 and len(image_files) > 0: # Only show warning if some images exist but not 4
             self._update_status(f"Warning: Subfolder {self.current_subfolder_name} has {len(image_files)} images, expected 4. Displaying up to 4.")

        subfolder_annotations_from_file = self.annotations_data.get(self.current_subfolder_name, {})

        for i in range(min(len(image_files), 4)): 
            img_name = image_files[i]
            img_path = os.path.join(full_subfolder_path, img_name)
            canvas = self.image_canvases[i]
            
            try:
                img = Image.open(img_path)
                self.current_pil_images[i] = img # Store original PIL image
                
                # Resize to the target display size for canvas content
                img_resized = img.resize(IMAGE_DISPLAY_SIZE, Image.LANCZOS)
                photo_img = ImageTk.PhotoImage(img_resized)

                self.current_photo_images[i] = photo_img 
                self.current_image_names[i] = img_name
                
                canvas.delete("all") # Clear previous content (like "No Image" text or error)
                # Draw image at (0,0) in canvas's coordinate system
                canvas.create_image(0, 0, anchor=tk.NW, image=photo_img, tags="current_image") 
                canvas.config(bg="white") 
                
                # Set scrollregion to the dimensions of the displayed image
                img_width, img_height = img_resized.size
                canvas.config(scrollregion=(0, 0, img_width, img_height))

                self.canvas_drawing_state[canvas]['image_name'] = img_name
                
                self.current_subfolder_active_annotations[img_name] = subfolder_annotations_from_file.get(img_name, []).copy() 
                self.drawn_bbox_ids_on_canvas[img_name] = []

                for bbox in self.current_subfolder_active_annotations[img_name]:
                    # Coordinates are already relative to the image's 0,0
                    rect_id = canvas.create_rectangle(bbox, outline=BBOX_COLOR, width=BBOX_WIDTH)
                    self.drawn_bbox_ids_on_canvas[img_name].append(rect_id)

            except Exception as e:
                self._update_status(f"Error loading image {img_name}: {e}")
                canvas.delete("all")
                nominal_width, nominal_height = IMAGE_DISPLAY_SIZE
                canvas.config(scrollregion=(0, 0, nominal_width, nominal_height), bg="lightgray")
                canvas.create_text(nominal_width // 2, nominal_height // 2, text=f"Error loading\n{img_name}", fill="red", justify=tk.CENTER, anchor=tk.CENTER)
                self.current_image_names[i] = None 
                self.current_pil_images[i] = None
                self.current_photo_images[i] = None


        # For any remaining canvas slots (if fewer than 4 images were loaded or errors occurred)
        for i in range(len(image_files), 4): # This loop covers slots not filled by valid images
            canvas = self.image_canvases[i]
            # Check if already handled by error case or needs a "No Image" placeholder
            if self.current_image_names[i] is None: # If not successfully loaded or error displayed
                canvas.delete("all")
                nominal_width, nominal_height = IMAGE_DISPLAY_SIZE
                canvas.config(scrollregion=(0, 0, nominal_width, nominal_height), bg="lightgray")
                canvas.create_text(nominal_width // 2, nominal_height // 2, text="No Image", fill="black", anchor=tk.CENTER)
                
                # Ensure state is clean for this empty slot
                self.current_pil_images[i] = None
                self.current_photo_images[i] = None
                if canvas in self.canvas_drawing_state:
                    self.canvas_drawing_state[canvas]['image_name'] = None

        self.unsaved_changes = False 
        self._update_button_states()


    def _update_button_states(self):
        has_subfolders = bool(self.subfolders)
        can_go_next = has_subfolders and self.current_subfolder_index < len(self.subfolders) - 1
        can_go_prev = has_subfolders and self.current_subfolder_index > 0

        self.btn_save.config(state=tk.NORMAL if has_subfolders else tk.DISABLED)
        self.btn_save_next.config(state=tk.NORMAL if can_go_next else tk.DISABLED)
        self.btn_clear.config(state=tk.NORMAL if has_subfolders else tk.DISABLED)
        self.btn_prev.config(state=tk.NORMAL if can_go_prev else tk.DISABLED)

    def _on_canvas_press(self, event, canvas):
        state = self.canvas_drawing_state[canvas]
        if state['image_name'] is None: 
            return
        
        # Convert visible canvas coordinates to scrollable content coordinates
        state['start_x'] = canvas.canvasx(event.x) 
        state['start_y'] = canvas.canvasy(event.y)
        # Create a temporary dashed rectangle for drawing
        state['active_rect_id'] = canvas.create_rectangle(
            state['start_x'], state['start_y'], state['start_x'], state['start_y'], 
            outline=BBOX_COLOR, width=BBOX_WIDTH, dash=(4, 2)
        )

    def _on_canvas_drag(self, event, canvas):
        state = self.canvas_drawing_state[canvas]
        if state['image_name'] is None or state['active_rect_id'] is None:
            return
        cur_x = canvas.canvasx(event.x)
        cur_y = canvas.canvasy(event.y)
        canvas.coords(state['active_rect_id'], state['start_x'], state['start_y'], cur_x, cur_y)

    def _on_canvas_release(self, event, canvas):
        state = self.canvas_drawing_state[canvas]
        if state['image_name'] is None or state['active_rect_id'] is None:
            return

        # Delete the temporary dashed rectangle
        canvas.delete(state['active_rect_id'])
        state['active_rect_id'] = None

        end_x = canvas.canvasx(event.x)
        end_y = canvas.canvasy(event.y)
        
        x1 = min(state['start_x'], end_x)
        y1 = min(state['start_y'], end_y)
        x2 = max(state['start_x'], end_x)
        y2 = max(state['start_y'], end_y)

        if abs(x1 - x2) < 5 or abs(y1 - y2) < 5:
            self._update_status("Box too small, not added.")
            return

        # Clamp coordinates to be within the image boundaries (0 to IMAGE_DISPLAY_SIZE-1)
        # IMAGE_DISPLAY_SIZE is the size of the image content on canvas (img_resized)
        img_w, img_h = IMAGE_DISPLAY_SIZE 
        x1 = max(0, min(x1, img_w -1))
        y1 = max(0, min(y1, img_h -1))
        x2 = max(0, min(x2, img_w -1))
        y2 = max(0, min(y2, img_h -1))
        
        if abs(x1 - x2) < 5 or abs(y1 - y2) < 5: # Check again after clamping
            self._update_status("Box too small after clamping, not added.")
            return

        bbox_coords = [int(x1), int(y1), int(x2), int(y2)]
        final_rect_id = canvas.create_rectangle(bbox_coords, outline=BBOX_COLOR, width=BBOX_WIDTH)

        current_image_name = state['image_name']
        if current_image_name not in self.current_subfolder_active_annotations:
            self.current_subfolder_active_annotations[current_image_name] = []
        self.current_subfolder_active_annotations[current_image_name].append(bbox_coords)
        
        if current_image_name not in self.drawn_bbox_ids_on_canvas:
            self.drawn_bbox_ids_on_canvas[current_image_name] = []
        self.drawn_bbox_ids_on_canvas[current_image_name].append(final_rect_id)
        
        self.unsaved_changes = True
        self._update_status(f"Added bbox to {current_image_name}: {bbox_coords}")


    def _action_save_annotations(self, called_from_next=False):
        if not self.current_subfolder_name:
            self._update_status("No subfolder loaded to save.")
            return False 

        self.annotations_data[self.current_subfolder_name] = self.current_subfolder_active_annotations.copy() 
        self._write_json_file(ANNOTATIONS_FILE, self.annotations_data)
        
        self.progress_data["last_completed_subfolder_name"] = self.current_subfolder_name
        self._write_json_file(PROGRESS_FILE, self.progress_data)
        
        self.unsaved_changes = False
        if not called_from_next: 
            self._update_status(f"Annotations for {self.current_subfolder_name} saved.")
        return True 

    def _action_save_and_next(self):
        if self._action_save_annotations(called_from_next=True): 
            self._update_status(f"Annotations for {self.current_subfolder_name} saved. Moving to next.")
            if self.current_subfolder_index < len(self.subfolders) - 1:
                self._load_subfolder_by_index(self.current_subfolder_index + 1)
            else:
                self._update_status("All subfolders annotated!")
                messagebox.showinfo("Info", "You have reached the end of the subfolders.")
                self._update_button_states() 
        else:
            self._update_status("Could not save, cannot move to next.")


    def _action_clear_current_annotations(self):
        if not self.current_subfolder_name:
            self._update_status("No subfolder loaded to clear.")
            return

        is_anything_to_clear = False
        # Check active annotations data for the current subfolder
        for img_name_key in self.current_subfolder_active_annotations:
            # Check if the image is part of the current display
            if img_name_key in self.current_image_names and self.current_subfolder_active_annotations[img_name_key]:
                is_anything_to_clear = True
                break
        
        # Fallback: also check drawn items on canvas (should be consistent with above)
        if not is_anything_to_clear:
            for i, canvas in enumerate(self.image_canvases):
                img_name = self.current_image_names[i]
                if img_name and img_name in self.drawn_bbox_ids_on_canvas and self.drawn_bbox_ids_on_canvas[img_name]:
                    is_anything_to_clear = True
                    break
            
        if not is_anything_to_clear:
            self._update_status(f"No annotations to clear for {self.current_subfolder_name}.")
            return

        if messagebox.askyesno("Confirm Clear", f"Are you sure you want to clear all annotations for subfolder '{self.current_subfolder_name}'? This action cannot be undone until next save."):
            for i, canvas in enumerate(self.image_canvases):
                img_name = self.current_image_names[i] # Name of the image on this canvas
                if img_name: 
                    # Clear drawn rectangles from canvas
                    if img_name in self.drawn_bbox_ids_on_canvas:
                        for rect_id in self.drawn_bbox_ids_on_canvas[img_name]:
                            canvas.delete(rect_id) # Make sure to delete from the correct canvas
                        self.drawn_bbox_ids_on_canvas[img_name] = []
                    
                    # Clear annotation data for this specific image in the current subfolder's active data
                    if img_name in self.current_subfolder_active_annotations: 
                        self.current_subfolder_active_annotations[img_name] = []
            
            self.unsaved_changes = True 
            self._update_status(f"Cleared annotations for {self.current_subfolder_name}. Save to persist this.")

    def _action_previous_subfolder(self):
        if self.unsaved_changes:
            if not messagebox.askyesno("Unsaved Changes", "You have unsaved changes. Do you want to proceed to the previous subfolder without saving?"):
                return
        
        if self.current_subfolder_index > 0:
            self._load_subfolder_by_index(self.current_subfolder_index - 1)
        else:
            self._update_status("Already at the first subfolder.")
            messagebox.showinfo("Info", "You are at the first subfolder.")
        self._update_button_states()


    def _load_annotations_from_file(self):
        try:
            if os.path.exists(ANNOTATIONS_FILE):
                with open(ANNOTATIONS_FILE, 'r') as f:
                    self.annotations_data = json.load(f)
                self._update_status(f"Loaded annotations from {ANNOTATIONS_FILE}")
            else:
                self.annotations_data = {}
                self._update_status(f"{ANNOTATIONS_FILE} not found. Starting fresh.")
        except Exception as e:
            self.annotations_data = {}
            self._update_status(f"Error loading {ANNOTATIONS_FILE}: {e}. Starting fresh.")
            messagebox.showwarning("Warning", f"Could not load annotations file: {e}")

    def _load_progress_from_file(self):
        try:
            if os.path.exists(PROGRESS_FILE):
                with open(PROGRESS_FILE, 'r') as f:
                    self.progress_data = json.load(f)
                self._update_status(f"Loaded progress from {PROGRESS_FILE}")
            else:
                self.progress_data = {}
                self._update_status(f"{PROGRESS_FILE} not found. Starting fresh.")
        except Exception as e:
            self.progress_data = {}
            self._update_status(f"Error loading {PROGRESS_FILE}: {e}. Starting fresh.")
            messagebox.showwarning("Warning", f"Could not load progress file: {e}")

    def _write_json_file(self, filepath, data):
        try:
            with open(filepath, 'w') as f:
                json.dump(data, f, indent=2)
        except Exception as e:
            self._update_status(f"Error saving to {filepath}: {e}")
            messagebox.showerror("Error", f"Could not save to {filepath}: {e}")


    def _on_app_close(self):
        if self.unsaved_changes:
            dialog = ExitDialog(self.root)
            choice = dialog.get_choice()
            if choice == "save_exit":
                self._action_save_annotations()
                self.root.destroy()
            elif choice == "exit_no_save":
                self.root.destroy()
            # else "cancel", do nothing and dialog closes itself
        else:
            self.root.destroy()

if __name__ == '__main__':
    root = tk.Tk()
    app = AnnotationApp(root)
    root.mainloop()